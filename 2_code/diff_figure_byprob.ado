...
program diff_figure_byprob

	syntax [varlist (default=none)] [if],  stub(string) step(string) probs(string) save(string) [drop(string)]
	...
	foreach race in `race_groups' {
		foreach network in faf u  {
			foreach p in  `probs' {
				...
				* Run regression
				*-----------------
				reg `race'_turnout_adultpop  dm_`stub'_`network'`drop'_p`p'_s`step'_ `varlist' if `restriction'
				post `memhold' (_b[dm_`stub'_`network'`drop'_p`p'_s`step'_] / _se[dm_`stub'_`network'`drop'_p`p'_s`step'_]) (_se[dm_`stub'_`network'`drop'_p`p'_s`step'_] ) (`p') (`category_counter')
				...
				*-----------------
				...
				label define categories `category_counter'"`label_network',Probability `p'", modify
